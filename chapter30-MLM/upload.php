...
$query = "insert into mail values
          (NULL, '".$_SESSION['admin_user']."',
          '$subject', '$list', 'STORED', NULL, NULL)";
$result = $conn->query($query);
if (!$result) {
    echo "<p>Could not add mail details to DB.</p>";
    do_html_footer();
    exit();
}
...
if (!$mailid) {
    echo "<p>Could not get mailid.</p>";
    do_html_footer();
    exit();
}

// creating directory will fail if this is not the first message archieved
// that's ok
...
if (!mkdir('archive/'.$list.'/'.$mailid, 0700)) {
    echo "<p>Could not make directory.</p>";
    do_html_footer();
    exit();
}
...
while ($_FILES['userfile']['name'][$i] &&
    ...
    if ($_FILES['userfile']['size'][$i] > $max_size) {
        echo "<p>Problem: ".$_FILES['userfile']['name'][$i].
             " is over max_size ".$max_size." bytes.</p>";
        $i++;
        continue;
    }
    ...
    if (!is_uploaded_file($_FILES['userfile']['tmp_name'][$i])) {
        // possible file upload attact detected
        echo "<p>Someting funny happening with "
