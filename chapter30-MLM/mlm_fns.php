...
function load_list_info($listid) {
    ...
    $result = $conn->query($query);
    if (!$result) {
        echo "<p>Can not retrieve this list.</p>";
        return false;
    }
...
function get_archive($listid) {
    // return an array of the archived mail for this list
...
function subscribe($email, $listid) {
    ...
    $query = "insert into sub_lists values
              ('$email', '$listid')";
    $result = $conn->query($query);
    return $result;
}
...
function unsubscribe($email, $listid) {
    ...
    $query = "delete from sub_lists where email='$email'
              and listid='$listid'";
    $result = $conn->query($query);
    return $result;
}
...
function store_list($admin_user, $details) {
    if (!filled_out($details)) {
        ...
        if (!check_admin_user($admin_user)) {
            // how did this function get called by
            // somebody not logged in as admin ?
            return false;
        }
        if(!($conn=db_connect())) {
            return false;
        ...
        $query = "insert into lists values
                  (NULL, '".$details['name']."', '".$details['blurb']."')";
        $result = $conn->query($query);
        return $result;
    }
}
...
// send test message to the administrator, or real message to the whole list
function send($mailid, $admin_user) {
    ...
    $query = "select path, mimetype from images
              where mailid='".$mailid."'";
    $result = $conn->query($query);
    if (!$result) {
    ...
    if ($status == 'STORED') {
        ...
        while ($subscriber = $result->fetch_row()) {
            if ($subscriber[2] == 'H') {
                ...
                echo "<p>HTML mail sent to ".$subscriber[1].".</p>";
            } else {
                ...
                // mail($subscriber[1], $subject, $text,
                     ...
                     // <'.$admin_user.'>');
                // if Mail Server needs auth
                $sender->send($subscriber[1], $hdrarray, $text);
                echo "<p>Plain text mail sent to ".$subscriber[1].".</p>";
            }
            $count++;
        }
        $query = "update mail
