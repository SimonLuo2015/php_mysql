...
if (@$username || @$passwd) {
    if (login($username, $passwd)) {
        $status .= "<p style=\"padding-bottom: 100px\">Loggin in
                    successfully.</P>";
...
switch (@$action) {
    ...
    case 'select-account':
        ...
        if (($accountid) && (account_exists($_SESSION['auth_user'], $accountid))) {
            $_SESSION['selected_account'] = $accountid;
        }
        break;
    default:
        break;
...
/*******************************************
  Stage 2: headers
  Send the HTML headers and menu bar
  appropriate to current action
*******************************************/
if (@$action) {
...
/*******************************************
  Stage 3: body
  Depending on action, show appropriate
  main body content
*******************************************/
...
if (!check_auth_user()) {
    ...
    switch ($action) {
        ...
        // delete an account, show account setup page
        case 'store-settings':
        case 'account-setup':
        case 'delete-account':
        ...
        case 'hide-headers':
        case 'view-message':
            // if we have just picked a message from the list, or were looking
            // at a message and chose to hide or view headers, load a messsage
        ...
        case 'reply-all':
            ...
            break;
        case 'reply':
            // set to address as replay-to or from of the current message
            if (@!$imap) {
                $imap = open_mailbox($_SESSION['auth_user'],
                                     $_SESSION['selected_account']);
            }
        ...
        case 'forward':
            ...
            if ($imap) {
                $header = imap_header($iamp, $messageid);
                $body = add_quoting(stripslashes(imap_body($imap, $messageid)));
