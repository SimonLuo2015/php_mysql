...
function display_account_select($auth_user, $selected_account) {
  ...
  if($accounts>1) {
    echo'<select onchange=window.location=this.options[selectedIndex].value name=accountid>';
    if($selected_account=='')
    echo '<option value=0 selected="selected">Choose Account</a>';
    for($i = 0; $i<$accounts; $i++) {
      $account = get_account_settings($auth_user, $list[$i]);
      echo "<option value='index.php?action=select-account&accountid=".$list[$i]."'";
  ...
  <table bgcolor='#cccccc' cellpadding = 6 cellspacing = 0 border =0>
   ...
   <tr>
     ...
     <?php
       ...
       echo ' value="IMAP">IMAP</option> <option';
       if ($type == 'POP3') {
           echo ' selected="selected"';
       }
       echo ' value="POP3">POP3</option></select></td>';
     ?>
   </tr>
   <tr>
     <td>User Name:</td>
...
?>
  ...
  <tr>
    <td>
    <?php
    // extract contents betweent <body> and </body>
    // $onlybody1=explode('&lt;body&gt;',$message['body']);
    // $onlybody2=explode('&lt;/body&gt;',$onlybody1[1]);
    // echo html_entity_decode($onlybody2[0]);
    echo html_entity_decode($message['body']);
    ?>
    </td>
  </tr>
  </table>
