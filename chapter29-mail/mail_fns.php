...
function store_account_settings($auth_user, $settings) {
    if (!filled_out($settings)) {
        echo "<p>All fileds must be filled in. Try again.</p>";
        return false;
    } else {
        if ($settings['accountid'] > 0) {
            $query = "update accounts
                      ...
                      where accountid='".$settings['accountid']."'
                        and username='$auth_user'";
        } else {
            $query = "insert into accounts values
                      ('$auth_user', '".$settings['server']."', '".$settings['port']."',
        ...
        if ($conn = db_connect()) {
            ...
            if ($result) {
                ...
                echo "<p>Could not complete query.</p>";
                return false;
            }
        } else {
            echo "<p>Could not connect to database to store changes.</p>";
            return false;
        }
    }
}
...
function open_mailbox($auth_user, $accountid) {
    ...
    // $mailbox should be {hostname/protocol:port}boxname
...
function send_message($to, $cc, $subject, $message) {
    ...
    if (!$result) {
        ...
        // FIXME PHP mail() command does not support authentication
